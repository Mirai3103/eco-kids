...
export default function useTTS() {
    ...
    (audioUrl: string, onFinish?: () => void) => {
      player.replace({
        ...
        headers: {
          ...
          "Authorization":
            `Bearer ${Constants.expoConfig?.extra?.supabaseAnonKey}`,
        },
      });
  ...
  const playTTSOnline = React.useCallback(
    async (
      ...
      let audioUrl =
        `${Constants.expoConfig?.extra?.supabaseUrl}/functions/v1/text-to-speech?text=${text}&gender=${gender}&lang=${language}`;
      if (segmentId) {
